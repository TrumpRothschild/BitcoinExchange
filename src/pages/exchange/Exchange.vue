<template>
  <div class="container exchange" :class="skin">
    <div v-if="this.$store.state.ismobile" class="moblieMain">
      <div class="coin-menu mivu-input" style="margin:0">
          <div style="padding: 8px 10px;height:auto; margin-bottom:5px;">
            <Input search :placeholder="$t('common.searchplaceholder')" @on-change="seachInputChange" v-model="searchKey" />
          </div>
        </div>
        <div>
            <div class="symbol symbolMobile" style="border-bottom: 1px solid #27313e;">
               <div class="item" @click="currentCoinFavorChange">
                ...
                <div class="item" style="margin-left: -40px;" @click="moblieShow()" >
                    <Icon type="md-menu" style="font-size: 26px;color:#cccccc;" />
                </div>
            </div>
          <div class="symbol symbolMobile">
          <div class="item">
            <span class="text" :class="{buy:currentCoin.change>0,sell:currentCoin.change<0}" style="font-size: 22px; color: #E55A54!important;">{{currentCoin.close | toFixed(baseCoinScale)}}</span>
            <span class="text">≈ ￥{{currentCoin.usdRate*CNYRate | toFixed(2)}}</span>
            <span class="text" :class="{buy:currentCoin.change>0,sell:currentCoin.change<0}" style="color: #E55A54!important;">{{currentCoin.rose}}</span>
          </div>
          <div class="item">
          ...
          <div class="item">
            <span class="text ">{{currentCoin.high | toFixed(baseCoinScale)}}</span>

            <span class="text ">{{currentCoin.low | toFixed(baseCoinScale)}}</span>
            <span class="text ">{{currentCoin.volume}} {{currentCoin.coin}}</span>

          </div>
        </div>
        <div class="imgtable imgtableMoblie">
          ...
          <DepthGraph :class="{hidden:currentImgTable==='k'}" ref="depthGraph" style="width:100%"></DepthGraph>
          </div>
        </div>
        <div class="trade_wrap trade_wrapMoblie" style="overflow:hidden">
          <div class="trade_panel trade_panel_logout">
            ...
            <div class="trade_bd trade_bdMolieButton">
              <div class="TabButton">
                 <Button @click="isTabButton(1)" :class="isTabShow?'activeTab':'defuTab'">买入</Button>
                 <Button type="warning" @click="isTabButton(2)" :class="!isTabShow?'activeTab':'defuTab'">卖出</Button>
                
              </div>
               <div class="panel panel_buy" v-show="isTabShow">
                ...
                <div class="bd bd_limited" v-show="!showMarket">
                  <Form ref="formValidate">
                    ...
                    <div class="slider-wrap">
                      ...
                      <div class="slider-stop" v-for="(item, index) in sliderStep" :key="index" :style="'left: '+item+'%;'" @click="silderGo('sliderBuyLimitPercent',item)">
                ...
                <div class="bd bd_market" v-show="showMarket">
                  <Form ref="formValidate">
                    ...
                    <div class="slider-wrap">
                      ...
                      <div class="slider-stop" v-for="(item, index) in sliderStep" :key="index" :style="'left: '+item+'%;'" @click="silderGo('sliderBuyMarketPercent',item)">
                        <div class="slider-block"></div>
                      </div>
                    </div>
              ...
              <div class="panel panel_sell" v-show="!isTabShow">
                <div v-if="isLogin" class="hd hd_login">
                ...
                <div class="bd bd_limited" v-show="!showMarket">
                  <Form ref="formValidate">
                    ...
                    <div class="slider-wrap">
                      ...
                      <div class="slider-stop" v-for="(item, index) in sliderStep" :key="index" :style="'left: '+item+'%;'" @click="silderGo('sliderSellLimitPercent',item)">
                        <div class="slider-block"></div>
                      </div>
                    </div>
                ...
                <div class="bd bd_market" v-show="showMarket">
                  <Form ref="formValidate">
                    ...
                    <div class="slider-wrap">
                      ...
                      <div class="slider-stop" v-for="(item, index) in sliderStep" :key="index" :style="'left: '+item+'%;'" @click="silderGo('sliderSellMarketPercent',item)">
                        <div class="slider-block"></div>
                      </div>
                    </div>
...
-->
      ...
      <div class="center">
        <div class="symbol">
          <div class="item" @click="currentCoinFavorChange">
            <Icon v-if="currentCoinIsFavor" type="ios-star" color="#f0a70a" size="18" />
            <Icon v-else type="ios-star-outline" color="#f0a70a" size="18" />
          </div>
          <div class="item">
              <span class="coin">{{currentCoin.coin}}
                <small>/{{currentCoin.base}}</small>
    ...
    <div class="order" :style="this.$store.state.ismobile?'padding:0 15px':''">
      ...
      <div :style="this.$store.state.ismobile?'overflow:auto':'  '">
      <div class="table" :style="this.$store.state.ismobile?'width:1200px':'  '">
    ...
    <van-popup v-model="moblieShowdlg" closeable style="height:100%;width:100%;background: #192430;">


<div class="coin-menu" style="padding: 45px 0 0 0;">
          <div style="padding: 8px 10px;height:48px;">
            <Input search :placeholder="$t('common.searchplaceholder')" @on-change="seachInputChange" v-model="searchKey"/>
          </div>
          <div class="sc_filter">
          ...
          <Table @on-current-change="gohref" highlight-row id="USDT" v-show="basecion==='usdt'" :columns="coins.columns" :data="dataIndex" style="max-height:550px;overflow:auto"></Table>
          <Table @on-current-change="gohref" highlight-row id="BTC" v-show="basecion==='btc'" :columns="coins.columns" :data="dataIndex" style="max-height:550px;overflow:auto"></Table>
          <Table @on-current-change="gohref" highlight-row id="ETH" v-show="basecion==='eth'" :columns="coins.columns" :data="dataIndex" style="max-height:550px;overflow:auto"></Table>
          <Table @on-current-change="gohref" highlight-row v-show="basecion==='favor'" :no-data-text="$t('common.nodata')" id="collect" :columns="favorColumns" :data="dataIndex"></Table>
<!--
          <p v-show="basecion!='favor'" style="height:40px;line-height:40px;padding-left:10px;border-bottom:1px solid #26303d;font-size:14px;color:rgb(97, 119, 146);">创新版</p>
          <Table @on-current-change="gohref" highlight-row id="USDT2" v-show="basecion==='usdt'" :columns="coins.columns" :data="dataIndex2"></Table>
...
<style scoped lang="scss">
 @media screen and (max-width:768px){
  .imgtableMoblie .kline_container{
