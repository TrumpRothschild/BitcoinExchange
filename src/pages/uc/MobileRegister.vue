  ...
  <div class="login_form mob-login">
    <div class="login_right">
      <div style="color: #F90;margin-bottom: 60px;margin-top: 60px;">
        <h1 style="border-left: 5px solid #F90;padding-left: 10px;letter-spacing: 2px;line-height:30px;height:30px;">cbdcoincurrency.COM</h1>
        <p style="padding-left:15px;letter-spacing: 6px;">Strictly select global high-quality digital assets</p>
      ...
      <Form v-if="allowRegister" ref="formInline" :model="formInline" :rules="ruleInline" inline>
        ...
        <FormItem prop="user">
          <Input type="text" v-model="formInline.user" :placeholder="key">
            ...
            <select v-model="country" slot="prepend" style="width: 65px;border-bottom: 1px solid #27313e;" class="countryselect">
    ...
    <div class="app_bottom_reg">
        ...
        <div class="right_btn_wrap"><router-link to="/" class="right_btn">{{$t("cms.download")}}</router-link></div>
      </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      window.document.title = (this.lang == "简体中文" ? "新用户注册 - " : "New Register - ") + "cbdcoincurrency | cbdcoincurrency offical website - global currency trading platform | global digital currency trading platform";
      // this.getAreas();
      // this.initGtCaptcha();
    },
    initGtCaptcha() {
