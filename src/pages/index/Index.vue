<template>
  <div>
    <div id="fullpage">
      <div style="display:none;background-image: linear-gradient( 135deg, #FD6585 10%, #0D25B9 100%);text-align: center;height:30px;line-height:30px;letter-spacing: 1px;">
        <a href="https://www.cbdcoincurrency.com/#/" target="blank" style="float:right;color: #FFF;margin-right:15px;">体验白蓝主题版本</a>
      </div>

      <div id="pagetips" style="border-bottom:1px solid rgb(28, 39, 58);" v-if="!ismobile">
        <div class="topnav">
          <div class="carl">
        ...
        <!-- <div v-if="lang==='简体中文'"> -->
        <div class="spin-wrap banner-panel" v-if="!ismobile">
          ...
          <div class="activity-list" v-show="picShow">
            <div class="swiper-container-pc swiper-container" id="swiper_container">
      ...
      <div class="mobile-banner" v-if="!!ismobile">
        <div id="swiper_container_m" class="swiper-container swiper_container_m">
              <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="(item,index) in picList" :key="index">
      ...
      <!--mobile banner end-->
      <!--mobile topnav  start-->
      <div class="topnav-m" v-if="!!ismobile">
        <div class="notice">
          <van-notice-bar left-icon="volume-o"   :scrollable="false"  color="#898e94" background="#141f2b" >
          <van-swipe
           loop
            class="notice-swipe"
            ...
            :autoplay="3000"
          >
            <van-swipe-item  v-for="(item,index) in FAQList" :key="index">
              <div class="cal_content">
                <span></span>
                <router-link target="_blank" style="color:#898e94;" :to="{path: '/announcement/' + item.id}">[ {{item.createTime}} ] {{strde(item.title)}}</router-link>
              </div>
            </van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        ...
        <div class="more">
              <router-link to="/announcement/0" target="_blank" class="gold">{{$t('common.more')}}</router-link>
        </div>
      </div>
      <!--mobile topnav  end-->
      <!--mobile ad start-->
      <div class="ad" v-if="!!ismobile">
       <div class="fontad">
          <div class="p">{{$t("common.slogan")}}</div>
          <div class="p">{{$t("common.subslogan")}}</div>
       </div>
      </div>
      <!--mobile ad end-->
      <div id="pagetips" style="background: #172636;" v-if="!ismobile">
          ...
          <div class="agent-list">
            <div class="agent-item">
              <div class="agent-img">
                ...
                <img v-else src="../../assets/images/new_1cny.png"></img>
              </div>
      ...
      <!--手机端 pagetips  start-->
      <div id="pagetips" style="background: #141f2b!important;" v-if="!!ismobile" class="pagetips-m">
        <div class="agent-panel">
          <div class="title">
            <div class="gettingstart">{{$t('sectionPage.gettingstart')}}</div>
            <div class="tips">{{$t('sectionPage.officialstart')}}</div>
      ...
      <div style="width: 100%;background: #141e2c;display:none;">
        <router-link to="/announcement/118930" target="_blank" style="width: 100%;">
          <img src="https://modcurrency.oss-accelerate.aliyuncs.com/bannerimg.png" style="width: 72%;margin-left: 14%;margin-top: 20px;">
        </router-link>
      </div>
      <div class="section" style="padding: 0px 14%;padding-top:50px;background: #141e2c;" v-if="!ismobile">
      ...
      <!--trendPanel mobile start-->
        <div class="section trendPanel-m" style="background: #141e2c;" v-if="!!ismobile">
          <div ref="trendPanel">
            <div>
              <div style="font-size: 14px;opacity:0.8;color:#fff;">BTC/USDT Trend</div>
              <div style="color:#828ea1;"><span class="lastest-price" style="font-size:30px;">{{trendData.close | fixed2}}</span><span style="font-weight: bold;">/USDT</span></div>
            </div>
            <SvgIndex ref="svgIndexRef"  :width="800" :height="150" :values="lineValues"></SvgIndex>
            

            <ul>
              <li>Highest price: <span class="gold">{{trendData.highest}}</span></li>
              <li>Lowest price: <span class="gold">{{trendData.lowest}}</span></li>
              <li>Change(24H): <span class="gold">{{trendData.rose}}</span></li>
              <li>Volume(24H): <span class="gold">{{trendData.volume}}</span></li>
            </ul>
          </div>
        </div>
      <!--trendPanel mobile end-->
      <div class="section" id="page2">
        <div v-if="!!ismobile" class="msearch"><Input search :placeholder="$t('common.searchplaceholder')" @on-change="seachInputChange" v-model="searchKey"  /></div>
        <div class="page2nav">
          ...
          <ul class="brclearfix">
            ...
            <li style="float:right;padding-right: 6px;" v-if="!ismobile"><Input search :placeholder="$t('common.searchplaceholder')" @on-change="seachInputChange" v-model="searchKey"/></li>
          </ul>
        </div>
        <div class="ptjy">
          <Table @on-row-click="showExchange"  v-if="choseBtn==0" :columns="favorColumns" :data="dataIndex" class="tables" :disabled-hover="true" :loading="loading"  :no-data-text="$t('common.nodata')"></Table>
          <Table @on-row-click="showExchange"  v-if="choseBtn!=0" :columns="coins.columns" :data="dataIndex" class="tables" :disabled-hover="true" :loading="loading"  :no-data-text="$t('common.nodata')"></Table>
      <!--
      ...
      <div class="section" id="page5">
        <img src="../../assets/images/phone_img.png" alt="" style="margin-top:8%;">
        <div class="phone_image" v-if="!ismobile"></div>
        <ul class="download" v-if="!ismobile">
          <li class="qrcode">{{$t('description.scanqrcode')}}</li>
          <li class="wrapper">
            <div class="download_app">
              <img  :style="{'display':!ismobile? 'block' : 'none'}" src="../../assets/images/appdownload.png">
            </div>
            <div class="abstract">
              <div class="content">
                <span></span>
    ...
    <div class="app_bottom">
      ...
      <div class="right_btn_wrap"><router-link target="_blank" to="/" class="right_btn">{{$t("cms.download")}}</router-link></div>
    </div>
  </div>
</template>
<style scoped lang="scss">
  ...
  @media screen and (max-width:768px){
    ...
    #page2 .ptjy .tables .ivu-table .ivu-table-body .ivu-table-cell div span{
  ...
  #swiper_container_m .swiper-slide,#swiper_container_m .swiper-slide a,#swiper_container_m .swiper-slide a img{
    width:100%;
    height:auto;
    display:block;
  }
  ...
  #page2 .page2nav .brclearfix li.active{
    background: none !important;
  }

  #page2 .page2nav .brclearfix li{
    font-size: 16px!important;
    padding: 0px 10px!important;
    margin-right:5px;
  }

  #page2 .page2nav .brclearfix{
  ...
  #page4 ul li div img,#page4 ul li div{
...
// console.log(params.row.usdRate, CNYRate);
  ...
  methods: {
    ...
    initSwiper(){
      var ss = new Swiper ('.swiper-container-pc', {
        loop: true,
        autoplay: true,
        pagination: {
     ...
     /****手机端轮播init  start**/
      var swiper = new Swiper ('.swiper_container_m', {
    ...
    /****手机端轮播init  end**/
      ...
      //var comtainermobile = document.getElementById('swiper_container_m');
...
<style scoped lang="scss" >
#page2 .page2nav .brclearfix li:hover{
  background:none!important;
}
...
.lastest-price{
  background:-webkit-linear-gradient(left,#F2A800,#FFCE60 25%,#E4A923 50%,#F2A800 75%,#CF9615);
