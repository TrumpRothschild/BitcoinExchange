<template>
  <div class="envelope">
    ...
    <div style="position:absolute;top: 40px;width: 100%;text-align:center;padding-top: 50px;">
      ...
      <p style="text-align:center;padding: 15px 0px;font-size:14px;letter-spacing:1px;width:100%;color:rgb(227, 205, 187);">Received: {{envelopeInfo.receiveCount}}/{{envelopeInfo.count}}</p>
    ...
    <p style="margin-top: -80px;margin-bottom: 80px;text-align:center;color: rgb(255, 136, 79);font-size:13px;">The red envelope is sent via cbdcoincurrency(cbdcoincurrency.COM)</p>
    ...
    <router-link style="font-size:14px;text-align:center;color: #EEE;margin-top:15px;text-decoration:underline;" to="/app">© cbdcoincurrency.COM | 下载APP</router-link>
    </p>
...
export default {
  data() {
    return {
      spinShow: false,
      hasReceived: false,
      inviteLink: "https://www.cbdcoincurrency.com/envelope/",
      envelopeNo: "",
      country: "",
      ...
      hasInviteUser: false,
      inviteUserId: "***********",
      inviteUserAvatar: "https://modcurrency.oss-accelerate.aliyuncs.com/defaultavatar.png",
      envelopeInfo: {
        id: 0,
        name: "**********",
        totalAmount: "0.00",
        ...
        invite: 0,
        type: 0,
        logo: "https://modcurrency.oss-accelerate.aliyuncs.com/applogo.png",
        bgImage: "https://modcurrency.oss-accelerate.aliyuncs.com/redenvelope.png",
        state: 0,
        detail: "留给未来一个暴富的可能"
      },
      formInline: {
    ...
    getEnvelope(){
        ...
        this.$http.post(this.host + "/uc/redenvelope/query", param).then(res => {
          ...
          if (res.status == 200 && res.body.code == 0) {
            ...
            window.document.title = "【" + this.envelopeInfo.totalAmount + " " + this.envelopeInfo.unit + "】" + this.envelopeInfo.name + "  — cbdcoincurrency | cbdcoincurrency offical website - global currency trading platform | global digital currency trading platform"
            this.getEnvelopeDetailList();
          } else {
              this.$Message.error(res.body.message);
          }
