      ...
      <div class="bill_flow_box">
        <div class="rightarea-con">
          <div class="form-group group-pc">
           ...
           <div class="form-group group-m">
            <div class="boxm">
              <span>
                {{$t('uc.finance.record.start_end')}} ：
              </span>
              <DatePicker class="boxmdiv"  v-model="rangeDate" @on-change="changedate" format="yyyy-MM-dd" type="daterange" @on-clear="clear"></DatePicker>
            </div>
            <!--<DatePicker v-model="startDate" type="date"></DatePicker>-->
            <!--<span>-->
            <!--{{$t('uc.finance.record.to')}}-->
            ...
            <div class="boxm">
              <span>{{$t('uc.finance.record.symbol')}} ：</span>
              <Select v-model="coinType" class="boxmdiv" @on-change="getAddrList" clearable :placeholder="$t('common.pleaseselect')">
                <Option v-for="item in coinList" :value="item.unit" :key="item.unit">{{ item.unit }}</Option>
              </Select>
              </div>
              <div class="boxm">
                ...
                <Select class="boxmdiv" v-model="recordValue" clearable  @on-change="getType" :placeholder="$t('common.pleaseselect')">
                  <Option v-for="item in recordType" :value="item.value" :key="item.value">{{ item.label }}</Option>
                </Select>
              </div>
            <Button type="warning" @click="queryOrder" style="padding: 6px 25%;background-color:#f0a70a;border-color:#f0a70a;    margin: 5% auto; display:block;">{{$t('uc.finance.record.search')}}</Button>
          </div>
          ...
          <div :style="{'margin': '10px','overflow': 'hidden'}">
              <div :style="{float: !!this.$store.state.ismobile ? '' : 'right'}">
