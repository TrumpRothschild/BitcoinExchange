  ...
  <div class="nav-rights">
    <div class="nav-right">
      <div class="bill_box">
        ...
        <section>
          <div :class='["action-box","open",this.$store.state.ismobile?"tableMoblie":"table-inner"]'>
            <!-- <i class="angle" style="right: 71px;"></i> -->
            <div class="action-inner">
              <div class="inner-left">
                <p class="describe">{{$t('uc.finance.recharge.symbol')+`${this.$store.state.ismobile?':':''}`}}</p>
              ...
              <div class="inner-box deposit-address">
                <p class="describe">{{$t('uc.finance.recharge.address')+`${this.$store.state.ismobile?':':''}`}}</p>
                <div class="title">
                  <Input v-model="qrcode.value" readonly style="" v-if="this.$store.state.ismobile"></Input>
                  <Input v-model="qrcode.value" readonly style="width: 400px;color:#8c979f;" v-else></Input>
              ...
              <!-- <div :class='[this.$store.state.ismobile?"innerMoblie":"inner-right"]'>
            ...
            <div class="action-content">
             ...
             <p  class="actionMobliep" v-if="this.$store.state.ismobile">{{$t('uc.finance.recharge.record')}}</p>
            <div :class='[this.$store.state.ismobile?"actionMoblie":"action-content"]'>
              <div class="action-body">
                <p class="acb-p1" v-if="!this.$store.state.ismobile">{{$t('uc.finance.recharge.record')}}</p>
...
.tableMoblie .innerMoblie{
 width: 100%;
 padding: 15px 0 5px;
 margin: 15px 0 0;
 overflow: hidden;
 border-top: 1px solid rgb(39, 49, 62);
}
.tableMoblie .innerMoblie a{
  background: #F0A70A;
  border-radius: 7px;
  width: 80%;
  color:#fff;
 ...
 .tableMoblie .actionMobliep{
    text-align: left;
        height: 40px;
    line-height: 40px;
    margin: 10px 0 0 0;
  }
 .tableMoblie .formBackground{
 background: #17212E;
 border-radius: 0 0 7px 7px;
 padding:  20px 0;
  }
  .tableMoblie .actionMoblie{
       padding: 0 0 10px;
        background: #17212E;
       border-radius:7px;
  }
        .tableMoblie .actionMoblie .actionMoblieBox{
         border-radius: 7px;
       }
     .tableMoblie .actionMoblie .acb-p1{
      text-align: left;
    }
     .tableMoblie .actionMoblie .actionMoblieBox{
      overflow: auto;
     }
   .tableMoblie .actionMoblie .actionMoblieTable{
      width: 660px;
    }
 .tableMoblie  .action-inner,.form-group-container,.form-group-container2{
  display: block;
  padding: 0;
 }
  .tableMoblie  .inner-left, .tableMoblie  .inner-box,.form-amount,.form-fee,.form-group{
    ...
    .tableMoblie  .inner-left>div,.tableMoblie  .inner-box>div,.form-address>div{
      width: 100%!important;
      margin-top: 2%!important;
      text-align: left;
    }
   .tableMoblie .describe,.label-amount,label{
    display: block;
    width: 100%;
  ...
  .tableMoblie .label-fr{
    text-align: left;
    font-size: 12px;
  }
    .tableMoblie .inner-box .form-address{
      display: block;
      width: 100%!important;
      margin: 0;
    }
     .tableMoblie .form-group{
      display: block;
      padding:0;
      width: 100%;
      margin: 0 0 15px 0;
    }
  .tableMoblie .action-foot{
   padding: 0;
   width: 100px;
   margin-left: 70px;
 }
   .tableMoblie .action-inner{
    padding: 20px 5px;
    background: #17212E;
    border-radius: 7px;
  }
 ...
 margin-top: 15px;
   ...
   .tableMoblie  .action-content.pt10{
   background: #17212E;
border-radius: 7px;
padding: 0 10px 20px;
 }
