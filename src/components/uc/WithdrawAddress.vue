      ...
      <div class="bill_box_address">
        ...
        <section>
          <div :class='[this.$store.state.ismobile?"tableMoblie":"table-inner"]'>
            <div class="action-inner">
              <div class="inner-left">
                ...
                <Select v-model="coinType" style="width:100px;margin-top: 10px;" size="large"  v-if="!this.$store.state.ismobile">
                 ...
                 <Select v-model="coinType" style="height:42px;margin-top: 0px;" size="large" v-else>
                  <Option v-for="item in coinList" :value="item" :key="item">{{ item }}</Option>
                </Select>
              </div>
              <div class="inner-box deposit-address mt25">
                <p class="describe">{{$t('uc.finance.withdraw.address')+`${this.$store.state.ismobile?':':''}`}}</p>
                <div class="title">
                  <Input v-model="withdrawAddr" style="width: 90%;margin-top:10px;" size="large" v-if="!this.$store.state.ismobile"></Input>
                  <Input v-model="withdrawAddr" style="height:42px;margin-top:0px;" size="large" v-else></Input>
                </div>
              </div>
              <div class="inner-box deposit-address mt25">
                <p class="describe">{{$t('uc.finance.withdraw.remark')}} / Memo{{`${this.$store.state.ismobile?':':''}`}}</p>
                <div class="title">
                  <Input v-model="remark" style="width:100%;" size="large" v-if="this.$store.state.ismobile"></Input>
                  <Input v-model="remark" style="width:100%;margin-top:10px;" size="large" v-else></Input>
                </div>
              </div>
               <div class="mt25 mtTitle"  v-if="this.$store.state.ismobile">
                <div class="title">
            ...
            <div class="btnbox" v-if="!this.$store.state.ismobile">
              <Button id="addrSubmit" @click='addAddr' size="large" style="width:86px;color:#fff;background:#f0a70a;border:1px solid #f0a70a;">{{$t('uc.finance.withdraw.add')}}</Button>
            </div>
            <p class="actionMobliep" v-if="this.$store.state.ismobile">{{$t('uc.finance.withdraw.addresslist')}}</p>
            <div :class='[this.$store.state.ismobile?"actionMoblie":"action-content"]'>
              <div class="action-body">
                <p class="acb-p1 describe" v-if="!this.$store.state.ismobile">{{$t('uc.finance.withdraw.addresslist')}}</p>
